...
// StegoBytes is required by remuxTsToFmp4 in browser / node
...
test('unfragmentFmp4 passes through structurally invalid bytes even with audioOnly', () => {
  const small = new Uint8Array([0, 0, 0, 8, 0x66, 0x74, 0x79, 0x70]);
  assert.equal(unfragmentFmp4(small, 0, { audioOnly: true }), small);
});
...
function createMockFmp4(sampleCount = 6, sampleDurationTicks = 90000) {
  ...
  new DataView(tfhdPayload.buffer).setUint32(4, 1);
