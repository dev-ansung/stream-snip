...
test('encodePcmToMp3 produces a strictly well-formed, frame-accurate MP3 bitstream (stereo)', () => {
  ...
  // 1s @ 44100Hz / 1152 samples-per-frame ~= 38 frames, plus a couple from
  // encoder flush/priming. Assert a tolerant-but-meaningful range rather
  // than pinning the exact count to lamejs's internal delay behavior.
...
// mp3-encoder.js's Web Audio decode step is browser-only (needs
// AudioContext). Node has neither AudioContext nor a real audio decoder,
// so these exercise the guard clauses that report that clearly instead of
// crashing on an undefined global; the actual PCM->MP3 encode path is
// covered above via encodePcmToMp3, which has no browser dependency.
