 ...
 * @param {{ trimStart?: number, trimEnd?: number, audioOnly?: boolean }} [options] - Options including trimStart, trimEnd, audioOnly
 * @returns {Uint8Array|null} - Progressive MP4 bytes, or null if audioOnly requested without audio track
 */
function unfragmentFmp4(bytes, duration = 0, options = {}) {
  ...
  try {
    ...
    // Boundary Invariant Guardrails: trimStart cannot exceed total media duration
    const refTrack = videoTrack || audioTrack;
    if (refTrack && refTrack.samples && refTrack.samples.length > 0 && refTrack.timescale > 0) {
      const totalTrackTicks = refTrack.samples.reduce((acc, s) => acc + s.dur, 0);
      const totalTrackDurationSec = totalTrackTicks / refTrack.timescale;

      if (trimStart >= totalTrackDurationSec && totalTrackDurationSec > 0) {
        console.warn(
          `[StegoTransmuxer] Invariant violated: trimStart (${trimStart}s) >= total track duration (${totalTrackDurationSec.toFixed(2)}s). Disabling edit list.`
        );
        trimStart = 0;
      }

      if (trimEnd > totalTrackDurationSec && totalTrackDurationSec > 0) {
        trimEnd = totalTrackDurationSec;
      }
    }
...
/**
 * @param {Uint8Array} tsBytes - Concatenated MPEG-TS segment bytes
 * @param {number} duration - Clip duration in seconds
 * @param {{ trimStart?: number, trimEnd?: number, audioOnly?: boolean }} [options] - Trimming options
 * @returns {Uint8Array}
 */
...
function transmuxTsToAudioMp4(tsBytes, duration = 0, options = {}) {
  ...
  if (!mux || !mux.mp4 || !mux.mp4.Transmuxer) {
    throw new Error('mux.js Transmuxer unavailable for audio extraction.');
  }
  ...
  const parts = [];
  transmuxer.on('data', (segment) => {
    if (segment.initSegment) parts.push(segment.initSegment);
  ...
  if (parts.length === 0) {
    throw new Error('No media found in stream for audio extraction.');
  }
  ...
  const audioOnlyMp4 = unfragmentFmp4(mergedMp4, duration, { ...options, audioOnly: true });
  if (!audioOnlyMp4) {
    throw new Error('No audio track could be extracted from this stream.');
  }
  return audioOnlyMp4;
}
