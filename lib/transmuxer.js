...
/**
 * Builds an elst (Edit List) FullBox for an MP4 track.
 * Instructs players to begin presentation at mediaTimeTrackTicks (in track timescale)
 * and play for segmentDurationMovieTicks (in movie timescale).
 *
 * @param {number} segmentDurationMovieTicks - Duration in movie timescale units
 * @param {number} mediaTimeTrackTicks - Starting offset in track timescale units
 * @returns {Uint8Array} - Complete elst box bytes
 */
function buildElst(segmentDurationMovieTicks, mediaTimeTrackTicks) {
  const b = new Uint8Array(16);
  const v = new DataView(b.buffer);
  v.setUint32(0, 1); // entry_count = 1
  v.setUint32(4, segmentDurationMovieTicks); // segment_duration
  v.setInt32(8, mediaTimeTrackTicks); // media_time
  v.setInt16(12, 1); // media_rate_integer = 1
  v.setInt16(14, 0); // media_rate_fraction = 0
  return createFullBox('elst', 0, 0, b);
}

/**
 * Wraps an elst box in an edts (Edit Box) container.
 *
 * @param {Uint8Array} elstBox
 * @returns {Uint8Array} - Complete edts box bytes
 */
function buildEdts(elstBox) {
  return createBox('edts', elstBox);
}
 ...
 * @param {object} options - { audioOnly: drop the video track and produce an audio-only MP4 }
 * @returns {Uint8Array|null} - Progressive MP4 bytes, or null if audioOnly was requested but no audio track exists
 */
function unfragmentFmp4(bytes, duration = 0, options = {}) {
  ...
  try {
    ...
    const videoTrack = options.audioOnly
      ? null
      : trackMeta[trackIds.find((id) => trackMeta[id].handlerType === 'vide')];

    if (options.audioOnly && !audioTrack) {
      return null;
    }
    ...
    const trimEnd =
      typeof options.trimEnd === 'number' && options.trimEnd > trimStart
        ? options.trimEnd
        : duration > 0 && trimStart > 0
          ? trimStart + duration
          : 0;

    // Prune trailing samples that occur after trimEnd
    if (trimEnd > 0) {
      for (const track of [videoTrack, audioTrack]) {
        if (!track || !track.samples || track.samples.length === 0) continue;
        const maxTicks = Math.round(trimEnd * track.timescale);
        let curTicks = 0;
        const pruned = [];
    ...
    const aTotalDur = audioTrack ? audioTrack.samples.reduce((s, x) => s + x.dur, 0) : 0;
    const fallbackDur = videoTrack
      ? vTotalDur / videoTrack.timescale - trimStart
      : aTotalDur / (audioTrack ? audioTrack.timescale : 1) - trimStart;
    const movieDuration =
      duration > 0
        ? Math.round(duration * movieTimescale)
        : Math.round(Math.max(0, fallbackDur) * movieTimescale);
    ...
    function buildElst(duration, mediaTime) {
      const b = new Uint8Array(16);
      const v = new DataView(b.buffer);
      v.setUint32(0, 1);
      v.setUint32(4, duration);
      v.setInt32(8, mediaTime);
      v.setInt32(12, 0x00010000);
      return createFullBox('elst', 0, 0, b);
    }
    ...
    function buildTrak(track, chunks, isVideo, chunkOffsets = null, edtsBox = null) {
      ...
      if (edtsBox) {
        return createBox('trak', tkhd, edtsBox, mdia);
      }
    ...
    const vEdts =
      trimStart > 0 && videoTrack
        ? buildEdts(buildElst(movieDuration, Math.round(trimStart * videoTrack.timescale)))
        : null;
    const aEdts =
      trimStart > 0 && audioTrack
        ? buildEdts(buildElst(movieDuration, Math.round(trimStart * audioTrack.timescale)))
        : null;

    const mvhd = buildMvhd();
    const dummyVTrak = videoTrack
      ? buildTrak(videoTrack, vChunks, true, dummyVOffsets, vEdts)
      : null;
    const dummyATrak = audioTrack
      ? buildTrak(audioTrack, aChunks, false, dummyAOffsets, aEdts)
      : null;
    ...
    const finalVTrak = videoTrack
      ? buildTrak(videoTrack, vChunks, true, vChunkOffsets, vEdts)
      : null;
    const finalATrak = audioTrack
      ? buildTrak(audioTrack, aChunks, false, aChunkOffsets, aEdts)
      : null;
...
// Remuxes raw TS bytes into a single fragmented MP4 (moov + moof/mdat pairs)
// via mux.js, without altering codec data.
// Returns { mergedMp4, muxAvailable }: mergedMp4 is null if mux.js is
// unavailable or the input produced no parseable media.
function remuxTsToFmp4(tsBytes) {
  const mux = getMuxJs();
  if (!mux || !mux.mp4 || !mux.mp4.Transmuxer) {
    return { mergedMp4: null, muxAvailable: false };
  }
  ...
  if (parts.length === 0) {
    return { mergedMp4: null, muxAvailable: true };
  }

  const mergedMp4 = StegoBytes.concatUint8Arrays(parts.map((p) => new Uint8Array(p)));
  return { mergedMp4, muxAvailable: true };
}

function transmuxTsToMp4(tsBytes, duration = 0, options = {}) {
  const { mergedMp4, muxAvailable } = remuxTsToFmp4(tsBytes);
  if (!mergedMp4) {
    if (!muxAvailable) {
      console.warn('mux.js Transmuxer not found, skipping MP4 transmuxing.');
    }
    return tsBytes instanceof Uint8Array ? tsBytes : new Uint8Array(tsBytes);
  }

  // Attempt to unfragment into a classic, progressive faststart MP4
  const unfragmented = unfragmentFmp4(mergedMp4, duration, options);
...
// Extracts just the audio track from raw TS bytes into a standalone
// progressive MP4 (playable as .m4a), for further decode/re-encode into
// formats like MP3. Throws if no audio track is present in the stream.
function transmuxTsToAudioMp4(tsBytes, duration = 0, options = {}) {
  const { mergedMp4, muxAvailable } = remuxTsToFmp4(tsBytes);
  if (!mergedMp4) {
    throw new Error(
      muxAvailable
        ? 'No media found while extracting audio (empty or invalid stream).'
        : 'Could not demux this stream (mux.js unavailable).'
    );
  }

  const audioMp4 = unfragmentFmp4(mergedMp4, duration, { ...options, audioOnly: true });
  if (!audioMp4) {
    throw new Error('No audio track found in this stream.');
  }

  return audioMp4;
}

const StegoTransmuxer = {
  transmuxTsToMp4,
