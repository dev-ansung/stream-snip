  ...
  async downloadSegments(segments, headers = {}, onProgress = null, signal = null) {
    ...
    const worker = async () => {
      while (nextIndex < total) {
        ...
        while (attempts < maxAttempts && !success) {
          ...
          try {
            ...
            if (attempts < maxAttempts) {
              // Exponential backoff with jitter: 400ms, 900ms, etc.
              const backoffMs = Math.floor(350 * Math.pow(2, attempts - 1) + Math.random() * 100);
        ...
        if (!success) {
          // If a segment returns 404 Not Found after all retries, or is missing on the CDN,
          // don't abort the entire download if most of the video is intact.
          // Store an empty chunk to preserve timeline ordering.
          const is404 = lastError?.status === 404 || String(lastError?.message).includes('404');
          if (is404) {
            console.warn(
              `[StegoClip:Downloader] Skipping missing segment ${segment.index} (HTTP 404) after ${maxAttempts} attempts: ${segment.url}`
            );
            results[currentIndex] = new Uint8Array(0);
            completed++;
            ...
            if (onProgress) {
              ...
              onProgress({
                ...
                elapsedSec,
                etaSec,
                warning: `Skipped missing segment ${segment.index}`
              });
            }
          } else {
            throw new Error(
    ...
    if (totalBytes === 0) {
      throw new Error('No valid segment data could be downloaded (all segments missing or empty).');
    }
